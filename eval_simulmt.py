...
def waitk_shard(model, tokenizer, shard, tgt_lang, k, max_new_tokens, show):
    ...
    for idx, src in tqdm(shard, desc=f"wait-{k}", disable=not show):
        ...
        al_val = average_lagging(trace, len(src.split()))
        laal_val = laal(trace, len(src.split()))
        out.append((idx, hyp, al_val, laal_val))
    return out
...
def evaluate_policy(model, tokenizer, state, sources, references, tgt_lang,
    ...
    row = {"policy": tag, "k": (None if policy == "full" else k),
           "bleu": bleu, "comet": comet_score, "AL": mean_al, "LAAL": mean_laal,
           "n": len(hyps)}
    return row, records
        ...
        for direction in args.directions:
            for lc in args.langs:
                ...
                for policy, k in runs:
                    ...
                    laal_str = f"  LAAL={row['LAAL']:.2f}" if row.get("LAAL") is not None else ""
                    comet_str = f"  COMET={row['comet']:.4f}" if row.get("comet") is not None else ""
                    print(f"  {tag:<10} BLEU={row['bleu']:.2f}{comet_str}{al_str}{laal_str}")
...
def _print_table(summary):
    print("\n" + "=" * 88)
    print(f"{'dataset':<6}{'dir':<5}{'lang':<5}{'policy':<10}{'BLEU':>8}{'COMET':>8}{'AL':>8}{'LAAL':>8}")
    print("-" * 88)
    for r in summary:
        al = f"{r['AL']:.2f}" if r.get("AL") is not None else "-"
        laal_v = f"{r['LAAL']:.2f}" if r.get("LAAL") is not None else "-"
        comet_v = f"{r['comet']:.4f}" if r.get("comet") is not None else "-"
        print(f"{r['dataset']:<6}{r['direction']:<5}{r['lang']:<5}"
              f"{r['policy']:<10}{r['bleu']:>8.2f}{comet_v:>8}{al:>8}{laal_v:>8}")
